...
function openVisualSection(key){
 ...
 modal.innerHTML='<div class="viewer-panel single"><button class="viewer-close" aria-label="Fechar">×</button><div class="viewer-heading"><div><p class="eyebrow">MATERIAL ORIGINAL</p><h2>'+s.title+'</h2></div><span id="viewerCounter"></span></div><div class="single-stage"><button class="slide-arrow prev" aria-label="Página anterior">‹</button><figure id="viewerPage"></figure><button class="slide-arrow next" aria-label="Próxima página">›</button></div><div class="viewer-dots" id="viewerDots"></div><p class="viewer-help">Use as setas ou deslize para o lado.</p></div>';
